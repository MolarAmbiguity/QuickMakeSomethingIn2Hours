...
function Reset()
  ...
  char.item = 'none'
  char.health = 1
  char.x = (love.graphics.getWidth() + char.img:getWidth()) / 2 -- puts the character in the middle of the screen
  char.y = love.graphics.getHeight() - char.img:getHeight() * SCALE
...
function Gravity(object, stop, speed, dt) -- applies gravity to an object. Uses dt (delta time) to make sure everything is the same
	if stop == 0 then
    if object.y < love.graphics.getHeight() - object.img:getHeight() * SCALE then
...
function UmbrellaSpawn(umbrella)
  umbrella.x = math.random(0, love.graphics.getWidth())
  umbrella.y = - 200
  umbrella.timer = umbrella.timerMax
end
